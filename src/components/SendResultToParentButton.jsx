...
import SendResultToParentButton from '../components/SendResultToParentButton';
...
function StudentResultsPage() {
  ...
  const result = useMemo(() => {
    ...
    if (!raw) return { score: 0, correctAnswers: 0, totalQuestions: 0, quizTitle: '' };

    try {
      return JSON.parse(raw);
    } catch (_error) {
      return { score: 0, correctAnswers: 0, totalQuestions: 0, quizTitle: '' };
    }
  }, []);
  ...
  // Refresh user info from server so parentWhatsapp is up to date
  useEffect(() => {
  ...
  useEffect(() => {
    const loadLeaderboard = async () => {
      ...
      try {
        ...
        const res = await api.get(`/sessions/${s.id}/leaderboard`);
  ...
  const quizTitle = result.quizTitle || session?.quizTitle || session?.title || 'الاختبار';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      ...
      >
        ...
        {/* Send result to parent via WhatsApp */}
        {user?.role === 'student' && (
          <SendResultToParentButton
